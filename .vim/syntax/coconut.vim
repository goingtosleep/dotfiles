...
"""
syn keyword coconutNone	None
syn keyword coconutBoolean	False True
syn keyword coconutStatement	as assert break continue del exec global
syn keyword coconutStatement	lambda nonlocal print with yield
syn keyword coconutClassStatement	class pass nextgroup=coconutClassname skipwhite
syn keyword coconutFuncStatement	def cdef return nextgroup=coconutFunction skipwhite
syn keyword coconutReturn	return
syn keyword coconutConditional	elif else if
syn keyword coconutRepeat	for while
syn keyword coconutOperator in is
syn keyword coconutBooleanOperator	and or not
syn keyword coconutException	except finally raise try
syn match   coconutFunction	"\h\w*" display contained
syn match   coconutClassname	"\h\w*\((\h\w*)\)\?" display contained
...
syn match   coconutPipe	/\%(|>\||\*>\|<|\|<\*|\)/ display
syn match   coconutPipeKwargs "|\*\*>" display


""" Compose
syn match   coconutCompose	"\.\." display
syn match   coconutForwardCompose	"\.\.>" display

""" Chain
syn match   coconutChain	"::" display

""" Arrow functions
syn match   coconutAdd	    "+" display
syn match   coconutMul	    "*" display
syn match   coconutSub	    "-" display
syn match   coconutDiv	    "/" display
syn match   coconutMod	    "%" display
syn match   coconutArrow    "->" display
syn match   coconutQmark	"?" display
...
if !exists("coconut_no_builtin_highlight")
  ...
  "syn keyword coconutBuiltin	False True
...
"  " Brackets, Parenthesis, Curly Brackets
  syn match	coconutBraces	   "[{}\[\]]"
  syn match	coconutParens	   "[()]"
  syn match	coconutOpSymbols	   "=\{1,3}\|!==\|!=\|<\|>\|>=\|<=\|++\|+=\|--\|-="
  syn match   coconutLogicSymbols "\(&&\)\|\(||\)"

  HiLink coconutBraces	        	Braces
  HiLink coconutParens	        	PreProc
  HiLink coconutBranch           	Conditional
  HiLink coconutLogicSymbols        Boolean
  HiLink coconutClassStatement    	PreProc
  HiLink coconutFuncStatement    	PreProc
  HiLink coconutReturn    	        PreProc

  " Symbolic operators
  HiLink coconutAdd    	            Operator 
  HiLink coconutSub    	            Operator
  HiLink coconutMul    	            Operator
  HiLink coconutDiv    	            Operator
  HiLink coconutMod    	            Operator
  HiLink coconutBooleanOperator    	Operator
  HiLink coconutOpSymbols           Statement
   
  HiLink coconutPipe    	        Pipe
  HiLink coconutArrow		        Pipe
  HiLink coconutCompose		        Pipe
  HiLink coconutForwardCompose		Pipe
  HiLink coconutChain		        Pipe
  HiLink coconutPartial		        Pipe
  HiLink coconutPlaceholder		    Pipe
  HiLink coconutQmark		        Pipe
  HiLink coconutPipeKwargs		    Pipe
                        


  "" The default highlight links.  Can be overridden later.
  HiLink coconutNone     	Number
  HiLink coconutBoolean     Number
  HiLink coconutStatement	Statement
  HiLink coconutConditional	Conditional
  HiLink coconutRepeat		Repeat
  HiLink coconutOperator		Statement
  HiLink coconutException	Exception
  HiLink coconutInclude		Statement
  HiLink coconutDecorator	Define
  HiLink coconutCodePaththrough	Macro
  "HiLink coconutFunction		Function
  "HiLink coconutClassname		Class
