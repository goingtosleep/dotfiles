...
syn keyword imbaKeyword import await typeof extends super yield new in css require global imba
...
syn keyword imbaKeyword def class constructor tag nextgroup=imbaMethod skipwhite
...
syn match imbaComment "#.*" display
syn region imbaComment start=/\/\*/ end=/\*\//
syn region imbaComment start=/###/ end=/###/
hi def link imbaComment Comment
...
syn match imbaOther /\%(\S\s*\)\@<=[+\-*/%&|\^=!<>?.]\{-1,}\|[-=]>\|--\|++\|:/ display
hi def link imbaOther Operator

syn keyword imbaEvents click
hi def link imbaEvents Statement
